...
class MyDataset(Dataset):
    def __init__(self, max_len, fs, partition, wave_scp_srcs, wave_scp_mix, wave_scp_noise=None, dynamic_mixing=False, speed_list=None):
        ...
        self.speed_list = speed_list if speed_list else [1.0]
    ...
    def speed_aug(self, waveform):
        """Speed perturbation as data augmentation"""
        speed = random.choice(self.speed_list)
        if speed != 1.0:
            ...
            return resampled.view(1, -1), speed
        return waveform.view(1, -1), speed
    ...
    def _dynamic_mixing(self, key):
        ...
        # checking whether it is the same speaker (Libri2Mix format: s1_<spkid>_<bookid>_<uttid>_mix)
        while True:
            key_random = random.choice(list(self.wave_dict_srcs[0].keys()))
            # Extract speaker IDs - assuming Libri2Mix format
            try:
                spk1_key = key.split('_')[1]
                spk2_key = key.split('_')[3]
                spk1_random = key_random.split('_')[1]
                spk2_random = key_random.split('_')[3]
                # Check if speakers are different
                tmp1 = spk1_key != spk1_random
                tmp2 = spk2_key != spk2_random
                if tmp1 and tmp2: break
            except IndexError:
                # Fallback in case key format is different
                break
        ...
        for file in files:
            ...
            # Speed Augmentation
            if hasattr(self, 'speed_list') and self.speed_list and len(self.speed_list) > 1:
